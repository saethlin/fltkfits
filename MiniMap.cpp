...
void MiniMap::draw() {
    ...
    if (this->image.size() == 0) {
        ...
        // Draw the rectangle and count the overwritten points as damaged

        int x0 = floor(window()->w()-200.0 + (double)x*200/original_width);
        int y0 = floor((double)y * 200/original_height);
        int x1 = floor(x0 + ((window()->w()-200.0) * 200/original_width)) - 1;
        int y1 = floor(y0 + ((window()->h()-50) * 200/original_height)) - 1;
        ...
        for (auto i = x0; i <= x1; i++) {
            damaged_pixels.push_back({i, y0, clipped(i-(window()->w()-200), y0)});
            damaged_pixels.push_back({i, y1, clipped(i-(window()->w()-200), y1)});
        }

        for (auto i = y0; i <= y1; i++) {
            damaged_pixels.push_back({x0, i, clipped(x0-(window()->w()-200), i)});
            damaged_pixels.push_back({x1, i, clipped(x1-(window()->w()-200), i)});
        }
